...
export const PizzaCustomization = () => {
  ...
  const [selectedFlavor, setSelectedFlavor] = useState<PizzaFlavor | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const flavorsData = MockDataService.getPizzaFlavors(pizzaType.id);
    const extrasData = MockDataService.getPizzaExtras();

    setFlavors(flavorsData);
    setExtras(extrasData);

    if (flavorsData.length > 0) {
      setSelectedFlavor(flavorsData[0]);
    }
  }, [pizzaType, navigate]);
